...
def run_read_from_netbox_test_case_fixture():
    ...
    def _run_read_from_netbox_test_case(
        ...
        for name in mock_sheet_names:
            ...
            mock_read_excel_calls.append(call().to_dict())

        mock_pd.read_excel.assert_has_calls(mock_read_excel_calls)
        assert mock_pd.read_excel.return_value.to_dict.call_count == len(
            mock_sheet_names
        )
...
def test_read_from_netbox_with_one_sheet_one_row(run_read_from_netbox_test_case):
    ...
    read_excel_return = [{"FQDN": {1: "fqdn1"}, "IDRAC IP": {1: "idrac_ip1"}}]
...
def test_read_from_netbox_one_sheet_many_rows(run_read_from_netbox_test_case):
    ...
    read_excel_return = [
        {"FQDN": {1: "fqdn1", 2: "fqdn2"}, "IDRAC IP": {1: "idrac_ip1", 2: "idrac_ip2"}}
    ]
...
def test_parse_args(mock_argparse):
    ...
    mock_argparse.ArgumentParser.return_value.parse_known_args.return_value = (
        mock_expected_args,
        NonCallableMock(),
    )
    ...
    mock_ap.add_argument.assert_has_calls(
        [
            ...
            call(
                ...
                default="output.txt",
                nargs="?",
                type=mock_argparse.FileType.return_value,
            ),
            call(
                "-r",
                "--reverse",
...
class TestCheckReachability(unittest.TestCase):
    ...
    def test_check_reachability(self, mock_is_reachable_dns, mock_is_reachable_ip):
        ...
        reachable_ips, unreachable_ips, reachable_dns, unreachable_dns = (
            check_reachability(ips_found, dns_found)
        )
